...
function PasswordModal({ open, onClose, onConfirm }: TPasswordModalProps) {
  ...
  const confirm = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (value === password) {
      sessionStorage.setItem('VicMSA-pwd', password);
  ...
  return (
    <Dialog open={open} onClose={close} fullScreen={fullScreen}>
      ...
      <DialogContent sx={{ pt: 9.5, px: 6, pb: 14 }}>
        ...
        <Typography component="p" variant="subtitle1" color="textPrimary" mt={4} mb={5}>
          {`Don't worry if you don't have the password; I'll be glad to meet you and show you my work  😃`}
        </Typography>
        <form id="password-form" noValidate autoComplete="off" onSubmit={confirm}>
                ...
                <FilledInput
                  ...
                  endAdornment={
                    <InputAdornment position="end">
                      <Button color="secondary" size="small" sx={{ fontSize: 12 }} onClick={togglePasswordVisibility}>
                        {showPassword ? `Hide` : `Show`}
                      </Button>
                    </InputAdornment>
                  }
                  aria-describedby="password-input-error-text"
                  onChange={resetPasswordStatus}
                />
                <Zoom in={passwordIsWrong}>
                  <FormHelperText id="password-input-error-text">{`Wrong password`}</FormHelperText>
                </Zoom>
              </FormControl>
            </Grid>
